...
/*: "USD" :*/
fileprivate let k_needValue:[Character] = ["U","S","D"]
...
class GlobalEqual: NSObject {
    //: static let shared = AppAdjustManager()
    static let shared = GlobalEqual()
    ...
    func status() {
        ...
        let adjustConfig = ADJConfig(appToken: dataEarthString, environment: environment)
        ...
        GlobalEqual.adjust(token: constTextDismissState)
    }
}
...
extension GlobalEqual: AdjustDelegate {
    /// 获取设备id
    //: class func getAdjustAdid() -> String {
    class func show() -> String {
    ...
    class func adjust(token: String) {
    ...
    class func indicatorWith(token: String, count: Double) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: event?.setRevenue(count, currency: "USD")
        event?.setRevenue(count, currency: (String(k_needValue)))
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
    }
    ...
    class func confirm(token: String) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
