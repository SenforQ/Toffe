...
class OverdoPing {
    ...
    static let shared = OverdoPing()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func writtenRecordMagnitude(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func make(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
    ...
    func willProduct(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
